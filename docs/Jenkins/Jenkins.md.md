...
## Environment  
Permet de définir des variables dans une pipeline déclarative  
```
pipeline {
    agent any
    environment {
...
## AWS Plugin   
1. Installer le plugin `Pipeline: AWS Steps Version`  
2. Aller dans `Manage jenkins > Credentials > System > Global credentials (unrestricted)`  
3. Ajouter un nouveau credential **AWS**  
   a- `ID :` Choisir un nom qui sera utilisé plus tard dans la pipeline  
   b- AccessKey  
   c-  SecretKey  
Exemple de **pipeline**  
```
...
                    dir('infra') {
                    sh 'echo "=================Terraform Init=================="'                          sh 'terraform init'
                }
            }
        }
}                                                                                      
...
## Params  
**Définir** les paramètres dans la pipeline  
```
pipeline {
    agent any
    parameters {
