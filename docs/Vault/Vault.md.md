...
`docker-compose up -d`  
Sur les followers :   
  Join existing cluster (api address : http://vault-node1:8200)  
## Dev mode  
...
Récupérer un secret-id avec un ttl  
`curl --request POST --header "X-Vault-Token: {{ vault_token}} " --data '{"metadata": "{}", "ttl": "20m"}' `  
...
## Revoke token en gardant les tokens orphans  

Ajouter la policy ci-dessous :  
```
