...
```
   Le **login** et **token**/mdp sera demandé au prochain `git clone`  et sera sauvegardé en mémoire pour les prochaines opérations `git clone` ou autres  
...
## Git clean  
Supprimer les fichiers pas encore add et commit  
```
git restore .
git clean -fd
```  

## Synchroniser nouvelle branche avec la master  
```
...
git merge main
```  
Git indiquera les fichiers en conflit (`git status`)  
Utiliser la cmd ci-dessous :
--ours pour garder la version de main
--theirs pour garder la version de la branche à merge
```
git checkout --ours (ou --theirs) .  
git add/rm <files> (les fichiers à git rm doivent être d'abord supprimés rm -f )
git commit -m "Résolution des conflits en gardant la version de COF-362" 
git push origin COF-362
```  

Ajouter les fichiers corrigés  (git add) et pousser les modifications (git push)  

## Ecraser la branche distante par la branche locale  e
```
git push origin add_tenant_vault-nnc --force
```  
  
## Git HTTPS credentials  
Modifier .git-credentials :  
`https://user:token@gitlab...`  
Ou bien  
```
git clone ...
user: $user
...
## Git 403  
### Problème  
Malgré les bons droits sur les projets et sous projets et un token avec les bons droits, on continue à voir une 403 forbiden sur les clone/pull/push que ce soit en HTTPS ou en SSH.  
### Solution  
Il faut désactiver le paramètre `Enforce SSH Certificagts` qui force une autorisation avec une paire de clé SSH dont la clé publique est signée par un owner  
